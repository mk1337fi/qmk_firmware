...
bool rgb_matrix_indicators_user(void) {
	if(host_keyboard_led_state().caps_lock) {
		for(uint8_t i = 0; i < ARRAY_SIZE(LETTER_LEDS); i++) {
			rgb_matrix_set_color(LETTER_LEDS[i], RGB_WHITE);
		}
		return true;
	}
    return false;
...
/*
static bool a_pressed, d_pressed;

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	switch(keycode) {
		case KC_A: if(d_pressed && record->event.pressed) unregister_code(KC_D); break;
		case KC_D: if(a_pressed && record->event.pressed) unregister_code(KC_A); break;
	}
	return true;
}
...
void post_process_record_user(uint16_t keycode, keyrecord_t *record) {
	switch(keycode) {
		case KC_A: a_pressed = record->event.pressed; if(d_pressed && !a_pressed) register_code(KC_D); break;
		case KC_D: d_pressed = record->event.pressed; if(a_pressed && !d_pressed) register_code(KC_A); break;
	}
}
*/
